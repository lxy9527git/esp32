...
#include "D:\esp32\esp-adf\components\esp_peripherals\driver\i2c_bus\i2c_bus.h"

/**< DEFINE---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------*/
#define WM8978_PORT                                 (I2C_NUM_0)
#define WM8978_SDA                                  (GPIO_NUM_13)
#define WM8978_SCL                                  (GPIO_NUM_12)

#define WM8978_SLAVE_ADDRESS                        (0x1A << 1)
...
static u8 WM8978_Write_Reg(u8 reg,u16 val)
{
	...
	IIC_BACK_Start();
	IIC_BACK_SendByte(WM8978_SLAVE_ADDRESS);
	if(IIC_BACK_WaitACK() != true)
	{
		ret++;
		portEXIT_CRITICAL(&lock);            //退出临界区
		printf("iic write fail:%d,%d!!!\n",reg,ret);
		return ret;
	}
	IIC_BACK_SendByte(data[0]);
	if(IIC_BACK_WaitACK() != true)
	{
		ret++;
		portEXIT_CRITICAL(&lock);            //退出临界区
	...
	if(ret == 0)WM8978_REGVAL_TBL[reg] = val;

	portEXIT_CRITICAL(&lock);            //退出临界区

	printf("iic write success:%d!!!\n",reg);
    ...
    // ret = i2c_bus_write_bytes(i2c_handle, WM8978_SLAVE_ADDRESS, &reg, 1, data, 2);

    // if(ret == 0)
    // {
    //     WM8978_REGVAL_TBL[reg] = val;
	// 	printf("iic write success !!!!\n");
	// 	return 0;
    // }
...
static int i2c_init()
{
    ...
    //     .sda_io_num    = WM8978_SDA,
    //     .scl_io_num    = WM8978_SCL,
    //     .master.clk_speed = 100000,
    // };
    // i2c_handle = i2c_bus_create(WM8978_PORT, &es_i2c_cfg);
...
static int RepeatCall(u8 reg,u16 val)
{
	int ret;
...
{
	return WM8978_Write_Reg(0,0);	//软复位WM8978
}

void WM8978_Setting(void)
{
	//以下为通用设置
	WM8978_Write_Reg(1,0X1B);	//R1,MICEN设置为1(MIC使能),BIASEN设置为1(模拟器工作),VMIDSEL[1:0]设置为:11(5K)
	WM8978_Write_Reg(2,0X1B0);	//R2,ROUT1,LOUT1输出使能(耳机可以工作),BOOSTENR,BOOSTENL使能
